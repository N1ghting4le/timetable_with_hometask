...
const Subject = ({ auditory, start, end, subgroup, subject, subjShort, type, weeks, teacher, hometask, weekIndex, dayIndex }) => {
    ...
    const sendHometask = async () => {
        ...
        if (!hometaskText && !text || hometaskText === text) {
            setProcess('idle');
            closeModal();
            return;
        }
        ...
        const changeOrDeleteHometask = async action => {
            ...
            if (oldHtList && oldHtList.length) {
                const newHtList = action === 'change' ? 
                                oldHtList.map(ht => ht.subject === subjShort && ht.teacher === lastName ? { ...ht, text } : ht) :
                                oldHtList.filter(ht => ht.subject !== subjShort || ht.teacher !== lastName);
